...
> | 7.5 — Editorial fingerprints (DESIGN_AMBITION) | 🔄 in progress | Locked 2026-05-02. **Round 1 ✅ done 2026-05-02** — `lib/folio.ts` + `Cue` component + edition stamp colophon; SiteHeader folio band (`SECTION ⟶ 01/24`); Cue marks on `/about` · `/awards` · `/productions/[slug]`; `2026 EDITION` in footer. Build clean. **Round 2** (production credits §3.B · theatre slate §3.F · two-geographies §3.G · premiere-mark cards N1, ~1.5–2d) — ships **after R2 / before D1**. **Round 3** (slate-strike §4.1.A + edition-frame §4.1.C, ~1d) — ships **after D1** behind `?gesture=off` flag. Cuts: §3.D, §3.J, §4.1.B. |
