...
Status: **drafted 2026-05-14**, awaiting approval to implement Step 4 (inline bake on admin upload).
...
- **Variants live alongside the source in R2.** Naming `<basename>.<W>.avif` (period-separated). Four widths: 420 / 600 / 828 / 1080.
...
- **The one improvement worth making**: bake the four AVIFs inline inside `/api/keystatic-asset` so admin uploads don't depend on a human remembering `npm run bake-variants` afterwards.
...
- The `/api/keystatic-asset` endpoint (`app/api/keystatic-asset/route.ts`) already implements raw R2 PUT with content-type detection, path sanitisation, ALLOWED_EXT validation, and 25 MB limit. It's battle-tested.
...
- The `imageSizes` config in `collections/Media.ts` (Track 1 of `PAYLOAD_IMAGE_VARIANTS_PLAN.md`) is already correct. Payload + sharp produces the same four widths, just under its own hyphen-naming (`portrait-w420.avif`). Consumers that read Media docs use `media.sizes.wXXX.url` directly — no path-substitution needed.
...
1. **Editor uploads** via either:
   - The "Upload" button in `ImagePathPreview` (Payload admin) → POSTs to `/api/keystatic-asset` → file lands at `productions/<slug>/<filename>` in R2.
   - Local workflow: drop the file into `public/productions/<slug>/<basename>.jpg`, then `npm run upload-images`.
2. **Variant bake** (currently manual, soon automatic): `npm run bake-variants -- --slug <slug>` → produces the four AVIFs alongside.
...
2. Payload + sharp auto-bakes the four widths on upload.
3. Consumer reads `media.sizes.wXXX.url` off the Media document.

## The one improvement worth making — inline bake on `/api/keystatic-asset`

### Problem

Step 2 of the standard flow above is manual. Sequence today:

1. Click "Upload" in admin → POST to `/api/keystatic-asset` → source lands in R2 ✅
...
Bake inline in the same request that uploads the source. Run sharp on the buffer we already have in memory, encode all four widths in parallel, PUT to R2 alongside. Same module the bulk script uses, no duplication.
...
3. **Patch `app/api/keystatic-asset/route.ts`** in POST, right after `await uploadToR2(buffer, r2Key, contentType)`:
...
- **Do not change the variant naming for Pipeline A.** Period-separated `<base>.<W>.avif` is set in stone — 1244 R2 keys plus consumer code already depend on it. If we ever need different widths, add them as new keys (e.g. `.1440.avif`); don't rename existing ones.
...
| 3. Patch `/api/keystatic-asset` POST to bake inline                            | same PR as step 2 | step 2  | One admin upload produces source + 4 variants in R2 |
...
3. **What about deletes?** The DELETE handler in `/api/keystatic-asset/route.ts` removes the source but leaves orphan variants. Recommendation: add a parallel `deleteVariants(sourceKey, client, bucket)` to the shared module, called from the DELETE handler. Small change, prevents R2 cruft.
