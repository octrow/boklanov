...
| S3 — Per-production OG images | ✅ done | `bda0628` |
| S4 — Analytics (PostHog) | ✅ done | `19c79ad` |
| S5 — DE chrome translations | ✅ done | (in-place) | All 44 keys already populated in messages/de.json from earlier session. Verified: all 8 translation namespaces complete, no EN=DE identical strings except "Links" which is correct German. | PostHog chosen (already installed). autocapture/pageview/recording all disabled. booking_cta_click only — data-ph-event delegation. No key = silently disabled. | `app/api/og/[slug]/route.tsx` (Node runtime, ImageResponse). Poster or oxblood typographic fallback. Lora title + JetBrains Mono chips + accent bar. Fonts: .woff from @fontsource packages. generateMetadata on production detail + about pages. | `Person` schema on `/about` (name, jobTitle, email, sameAs). `CreativeWork` on each production detail (inLanguage, productionCompany, dateCreated, audience, image, director). Inline `<script type="application/ld+json">`. |
