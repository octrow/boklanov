...
app/
  ...
  api/r2-asset/route.ts             multipart image upload + inline AVIF variant bake (Pipeline A)
...
Payload image upload: `/api/r2-asset` POST multipart `file` + `directory` is the R2 proxy used by
`components/admin/ImagePathPreview.tsx` (Payload custom field). After the source PUT succeeds it bakes the
four AVIF variants inline via `lib/image-variants.ts`, so editors don't need to remember `npm run bake-variants`.
DELETE on the same endpoint removes the source + the four orphan variants. Path validation rejects traversal,
overwrite, non-image, >25 MB. Renamed from `/api/keystatic-asset` once Payload became the only CMS in this branch.
