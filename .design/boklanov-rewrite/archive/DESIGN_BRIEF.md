...
| D11 | **Colour system:** **(c) photo-led** — chrome stays neutral, colour comes from production photos. **One reserved accent:** deep oxblood for booking CTAs and hover underlines only. _Superseded 2026-05-04: Phase 9v3.0 (`2827654`, 2026-05-03) replaced oxblood with v3 Bauhaus stage trio (vermillion / cobalt / mustard); discipline of one accent retained — `--accent` aliases the primary stage. Annotation backfilled here per `MAP.md` §5; same supersession enables the 2026-05-04 default-theme flip to "gorky" with paper as opt-in (see `DESIGN.md` §3)._ |
...
### 8.1 Supersessions (post-brief annotations)
...
### 8.2 v3 Plakat supersessions (2026-05-03)

Accepted on `design_v3` branch as part of "Plakat" direction. Full guard conditions + rollback triggers: `DESIGN_v3_PROPOSAL.md` §2. Mirrored into `DESIGN.md` §11 in Phase 9v3.8.

- _Superseded 2026-05-03: §2.1 Brand gradients — `<DuotonePoster>` SVG `<feColorMatrix>` two-tone filter on `featured: true` cards only. Shadows → `--accent-vermillion` or `--accent-cobalt`; highlights → `--paper`. `@supports filter:url()` gated; grayscale fallback. NEVER on body, headers, buttons, non-featured cards. Rollback: contrast on caption-over-image drops below 4.5:1 or appears outside poster context._
- _Superseded 2026-05-03: §2.2 Coloured chip pills (partial) — `<Sticker>` badge allowed for award/tour/form stamps only. Unbounded 600 ALL CAPS, fill one of `--accent-{vermillion,cobalt,mustard}`, `--ink-on-accent` text, no radius, optional `--shadow-plakat`. Max 1 per `<ProductionCard>`, max 3 per page. NEVER on body prose or metadata chips (those remain specimen mono labels per §8.1). Rollback: sticker on non-award/tour/form content; more than 1 per card; radius added._
- _Superseded 2026-05-03: §2.3 Hero video backgrounds — 3s autoplay loop on `/` only approved (≤800kb webm, `prefers-reduced-motion` swaps to first-frame poster image; never autoplay on other routes). Skipped in 9v3.5 — no qualifying clip available. Anti-pattern remains in force until a clip exists._
- _Superseded 2026-05-03: §2.4 Bento grids on home — `<FeaturedStrip>` asymmetric broken-grid on `/` only (1 large cols 1–7 + 2 medium + 3 small at ≥1024px; 2-col tablet; 1-col mobile). NEVER on `/productions`, `/about`, or any other route. Equal-size cells and card hover-lift remain banned. Rollback: bento on /productions or /about; equal-size cells; cards lift on hover._
- _Superseded 2026-05-03: §2.5 Animated gradient text — static `background-clip: text` gradient on `<SiteHero>` `<h1>` on `/` only (vermillion→cobalt→mustard, 135deg, fixed angle). `aria-hidden="true"` on gradient span; plain-text SR h1. NEVER animated. NEVER on second element. NEVER on other routes or chrome. Rollback: gradient animates; appears outside SiteHero; non-palette colours._
- _Superseded 2026-05-03: §2.6 `rounded-2xl shadow-xl` (partial extension of v2 §8.1) — `--shadow-plakat: 0 4px 0 0 var(--ink)` outset allowed on `<DuotonePoster>` only. Blur remains 0. No coloured shadow. Extends v2 form-chrome exception; all other shadow/radius restrictions unchanged. Rollback: blur > 0; outset shadow on non-poster element._
- _Superseded 2026-05-03: §2.7 Drop-shadow glow (partial extension of v2 §8.1) — `--shadow-plakat` flat-stack offset (blur 0, `--ink` colour) allowed on `<DuotonePoster>` and `<Sticker>`. Mimics screen-print misregister, not Tailwind card shadow. NEVER blur > 0, NEVER coloured beyond `--ink`. v2 specimen-rule (inset only on SpecimenPlate) unchanged. Rollback: blur > 0; shadow outside poster/sticker context._
- _Superseded 2026-05-03: §2.8 Coloured headers with white text — `<SectionStripe>` 2px accent band above `<Cue>` per-route (`/` vermillion, `/productions` cobalt, `/about` mustard, `/awards` vermillion, `/press` cobalt, `/archive` `--ink`, `/contact` vermillion). NEVER a fill block. NEVER beyond the Cue header. Title text stays `--ink`. Rollback: stripe becomes fill block; header text coloured; band > 2px._
- _Superseded 2026-05-03: §2.9 Marquee / ticker tape — `<TourTicker>` CSS `@keyframes` marquee (mono caps, `--ticker-speed 38s`). Pauses on `:hover` AND `prefers-reduced-motion` (static row). Used on `/` (between hero and Featured, accent=mustard) and production-detail when `tour[]` non-empty. Cities are past-tense staging locations. NEVER in body prose; NEVER adds gradient or colour. Rollback: appears in body prose; does not pause on reduced-motion._
