...
> | 4 — Frontend rebuild | ✅ done (11 / 11) | C1–C11 all committed. Layout shell, Cmd-K palette, theme toggle, lang switch shipped. 24 productions × 3 locales = 72 detail routes, plus per-locale chrome. Last commit: `ab2ce8b`. |
> | 5 — i18n + SEO + OG | ✅ done (5 / 5) | S1–S5 shipped. Sitemap with hreflang RU↔EN, robots, RSS (RU+EN), JSON-LD `Person` + `CreativeWork`, `app/api/og/[slug]`, PostHog `booking_cta_click` only, DE chrome translations complete. |
...
> | 6.6 — Post-R1 QA findings (Q1–Q8) | ✅ done | Q1–Q7 shipped 2026-05-02 across `10f951f` `b3bded7` `fdbae94` `99299de` `8dae0b2` `c7647bf`. Q8 (production-detail content depth — generic theatre extraction, new credits / premiere date / tickets URL extractors, Credits section per DESIGN §7.3) shipped 2026-05-02 same-day. 24 productions clean. Two follow-ups noted in TASKS.md: (a) two festival-in-plain-prose awards (cinderella, sugar-kid) need hand-overlay; (b) Roman to confirm RGISI / first-BTK milestone years; (c) 18 productions show no theatre line because their MD source has no `[Name](url)` theatre link — Roman can add `theatre:` in per-production `metadata.yml` overlay. |
...
> | 7.5 — Editorial fingerprints (DESIGN_AMBITION) | 🔄 in progress | **Round 1 ✅ done 2026-05-02** (`c7a1b50`) — folio band · Cue marks · edition stamp. **Round 2 ✅ done 2026-05-02** (`0bebf3c`) — credits `<dl>` + CREDITS cue (DA-2.A); theatre slate w/ PRODUCTION index + TOURING row (DA-2.B); `ГДЕ СТАВИЛ` on `/about` + home echo (DA-2.C); Plinth ON TOUR band + seed 9 cities in `metadata.yml` (DA-2.D); `PREM YYYY` on cards (DA-2.E); `about` i18n namespace; `tour[]` wired in `merge()` with overlay support. **Round 3** (DA-3.A slate-strike + edition-frame fallback, ~1d) — ships **after D1**, behind `?gesture=off`. Cuts: §3.D, §3.J, §4.1.B. |
> | 8 — Authoring handoff (Obsidian + R2) | ⏳ pending | Locked 2026-05-02 in `.design/boklanov-rewrite/CONTENT_WORKFLOW.md` (F + R2 + trust-on-publish + `metadata.yml` **folded into MDX frontmatter** — single source of truth per field). ~2.5 days: §6.1 vault layout, §6.2 R2 image migration, §6.3 fold overlay + retire Notion sync, §6.4 `AUTHORING.ru.md` mini-guide for Roman, §6.5 Cyrillic-only-Name orphan audit. Can run after Phase 7 cutover. |
