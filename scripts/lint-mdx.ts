...
function files(dir: string): string[] {
  return readdirSync(dir, { withFileTypes: true }).flatMap(e =>
    ...
    : e.name.endsWith('.mdx') || e.name.endsWith('.md') ? [join(dir, e.name)]
    : []
  )
}
...
for (const f of files(CONTENT_DIR)) {
  ...
  if (WIKILINK.test(text)) {
    console.error(`[lint-mdx] Obsidian wikilink found in ${f.replace(process.cwd(), '.')}`)
    console.error('  Fix: use standard markdown  ![alt](./image.jpg)  instead of  ![[image.jpg]]')
    errors++
  }
}

if (errors > 0) process.exit(1)
console.log(`[lint-mdx] OK — ${files(CONTENT_DIR).length} files checked, 0 wikilinks`)
