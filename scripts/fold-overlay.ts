...
function pick<T>(overlayVal: T | null | undefined, fmVal: T): T {
  ...
  if (Array.isArray(overlayVal) && (overlayVal as unknown[]).length === 0)
    return fmVal
  return overlayVal
}

function mergeGallery(fmGallery: Rec[], overlayGallery: Rec[]): Rec[] {
  ...
  return fmGallery.map((g) => {
    ...
    return {
      src: g.src,
      credit: pick(
        o.credit as string | null,
        (g.credit as string | null) ?? null
      ),
      caption: {
        ru: pick(
          oCap?.ru as string | null,
          (fmCap?.ru as string | null) ?? null
        ),
        en: pick(
          oCap?.en as string | null,
          (fmCap?.en as string | null) ?? null
        ),
        de: pick(
          oCap?.de as string | null,
          (fmCap?.de as string | null) ?? null
        )
      }
    }
  })
...
function applyOverlay(fm: Rec, overlay: Rec): Rec {
  ...
  return {
    ...
    title: {
      ...fmTitle,
      de: pick(
        ovTitle.de as string | null,
        (fmTitle.de as string | null) ?? null
      )
    },
    synopsis: {
      ...fmSynopsis,
      de: pick(
        ovSynopsis.de as string | null,
        (fmSynopsis.de as string | null) ?? null
      )
    },
    theatre: pick(
      overlay.theatre as Rec | undefined,
      (fm.theatre as Rec) ?? {}
    ),
    year: fm.year,
    premiereDate: pick(
      overlay.premiereDate as Rec | undefined,
      fm.premiereDate as Rec | undefined
    ),
    ticketsUrl: pick(
      overlay.ticketsUrl as string | null,
      (fm.ticketsUrl as string | null) ?? null
    ),
    ageRating: pick(
      overlay.ageRating as string | null,
      (fm.ageRating as string | null) ?? null
    ),
    durationMin: pick(
      overlay.durationMin as number | null,
      (fm.durationMin as number | null) ?? null
    ),
    ...
    credits: pick(
      overlay.credits as Rec | undefined,
      (fm.credits as Rec) ?? { ru: [], en: [] }
    ),
    poster: {
      ...fmPoster,
      credit: pick(
        ovPoster.credit as string | null,
        (fmPoster.credit as string | null) ?? null
      )
    },
    gallery: mergeGallery(fmGallery, ovGallery),
    videos: allVideos,
    awards: pick(
      overlay.awards as Rec[] | undefined,
      (fm.awards as Rec[]) ?? []
    ),
    press: (fm.press as Rec[]) ?? [],
    externalLinks: (fm.externalLinks as Rec[]) ?? [],
    techRider: pick(
      overlay.techRider as string | null,
      (fm.techRider as string | null) ?? null
    ),
    pressKit: pick(
      overlay.pressKit as string | null,
      (fm.pressKit as string | null) ?? null
    ),
    featured:
      typeof overlay.featured === 'boolean' ? overlay.featured : !!fm.featured,
    tags: (fm.tags as string[]) ?? [],
    tour: pick(
      overlay.tour as string[] | undefined,
      (fm.tour as string[]) ?? []
    )
  }
}
