...
| 8   | `.design/boklanov-rewrite/KEYSTATIC_R2_ONLY_PLAN.md`       | One-commit-per-save plan — drop GitHub commit branch from `/api/keystatic-asset`, R2 becomes upload target, replace `sync-r2.yml` with R2 → `public/` git backup workflow that re-establishes git as eventual source of truth |  ~210 |
